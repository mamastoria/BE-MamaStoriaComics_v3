...
app = FastAPI(
    title="Nano Banana Comic — Option B (Text in Image) + Portrait 3x3 + PDF + Viewer"
)
...
@app.get("/")
def home():
    # Prefer root index.html if you open directly; otherwise /static/index.html works too
    index_path = STATIC_DIR / "index.html"
    if not index_path.exists():
        return HTMLResponse(
            "<h3>UI belum ada</h3><p>Taruh file <code>static/index.html</code> untuk serve UI.</p>",
            status_code=200,
        )
...
@app.post("/api/render_part")
def api_render_part(req: RenderPartRequest):
    """
    Render satu part (langsung synchronous) untuk debug/manual.
    NOTE:
    - Endpoint ini tidak menghasilkan job_id, jadi preview-file /api/preview tidak tersedia untuk mode ini,
      kecuali kamu ubah core.render_part_payload agar menerima job_id dan menyimpan file.
    """
    try:
...
def api_job(job_id: str):
    ...
    resp: Dict[str, Any] = {
        ...
        # ✅ NEW flags (UI bisa pakai ini untuk show preview buttons)
...
def api_pdf(job_id: str, download: int = Query(0)):
    ...
    - default: inline (so it can render inside iframe/PDF.js)
...
# ============================================================
# ✅ PREVIEW FULL PAGE (BEFORE CUT)
# ============================================================
@app.get("/api/preview/{job_id}/{part_no}")
def api_preview(job_id: str, part_no: int):
    """
    Return full 3×3 page image before split.
    REQUIREMENT:
    - core.py harus menyimpan path image full-page ke job['part1'/'part2'] misalnya:
        job.part1.grid_path  atau  job.part1.part_full_path
    """
    core.cleanup_jobs()
    job = core.get_job(job_id)
    if not job:
    ...
    # Support either key name to be flexible with your core patch
...
# ============================================================
# PDF VIEWER + READ-ALONG — robust (no pdfjsLib undefined)
# ============================================================
...
def viewer(job_id: str):
    ...
    # NOTE:
    # - Use ESM import for PDF.js so we never depend on global "pdfjsLib".
    # - Skip empty TTS pages to prevent read-along from stalling.
