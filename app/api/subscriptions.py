...
async def purchase_subscription(
    ...
    if settings.DOKU_IS_PRODUCTION:
        ...
        # FIXED: Use internal Mock Payment Page for reliable testing without DOKU account
        payment_url = f"https://nanobanana-backend-1089713441636.asia-southeast2.run.app/api/v1/mock-payment/{order_id}"
...
async def mock_payment_page(order_id: str, db: Session = Depends(get_db)):
    """
    Mock payment page for development/demo purposes
    """
    transaction = db.query(PaymentTransaction).filter(
        PaymentTransaction.doku_order_id == order_id
    ).first()
    ...
    if not transaction:
        ...
        <html>
            <body style="font-family: sans-serif; text-align: center; padding: 50px;">
                <h1 style="color: red;">Transaction Not Found</h1>
                <p>Invalid Order ID: {order_id}</p>
            </body>
        </html>
    ...
    <html>
        <head>
            <title>Mock Payment - MamaStoria</title>
            ...
            <style>
                ...
                .details {{ text-align: left; background: #f9f9f9; padding: 1rem; border-radius: 8px; margin-bottom: 1.5rem; }}
                .row {{ display: flex; justify-content: space-between; margin-bottom: 0.5rem; }}
                .label {{ color: #666; }}
                .value {{ font-weight: 600; }}
                button {{ background: #2ecc71; color: white; border: none; padding: 1rem; width: 100%; border-radius: 8px; font-size: 1rem; font-weight: bold; cursor: pointer; transition: background 0.3s; }}
                button:hover {{ background: #27ae60; }}
            </style>
        </head>
        <body>
            <div class="card">
                <h1>MamaStoria Payment</h1>
                <p>Mock Payment Gateway (Dev Only)</p>
                
                <div class="amount">Rp {transaction.amount:,.0f}</div>
                
                <div class="details">
                    <div class="row">
                        <span class="label">Invoice:</span>
                        <span class="value">{transaction.doku_order_id}</span>
                    </div>
                    <div class="row">
                        <span class="label">Date:</span>
                        <span class="value">{transaction.created_at.strftime('%Y-%m-%d %H:%M')}</span>
                    </div>
                </div>
                
                <form action="/api/v1/mock-payment/{order_id}/pay" method="post">
                    <button type="submit">✅ SIMULATE SUCCESS PAYMENT</button>
                    <p style="font-size: 0.8rem; color: #888; margin-top: 1rem;">Clicking this will explicitly grant resources to your account.</p>
                </form>
            </div>
        </body>
    </html>
...
@router.post("/mock-payment/{order_id}/pay")
async def process_mock_payment(order_id: str, db: Session = Depends(get_db)):
    """
    ...
    if transaction.status == "success":
         return HTMLResponse(content="""
            <html>
                <body style="font-family: sans-serif; text-align: center; padding: 50px;">
                    <h1 style="color: orange;">Already Paid</h1>
    ...
    # ==========================================
    # LOGIC COPIED FROM payment_callback
    # ==========================================
    transaction.status = "success"
    ...
    return HTMLResponse(content="""
        <html>
            <body style="font-family: sans-serif; text-align: center; padding: 50px;">
                ...
                <p>Resources have been added to your account.</p>
                <p>You can close this window and create comics now.</p>
            </body>
        </html>
    """)
